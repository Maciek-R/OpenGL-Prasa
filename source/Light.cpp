...
void Light::drawLight() {
	...
	ModelMatrix = glm::rotate(ModelMatrix, Angle * (float)(PI / 180), glm::vec3(0, 1, 0));
...
void Light::move() {
	...
	posX = 5*glm::sin(przes * (float)(PI / 180)) + 6;
	posZ = 5*glm::cos(przes * (float)(PI / 180)) + 6;
...
void Light::serveTime() {
	...
	przes = 100*((float)(Now))/CLOCKS_PER_SEC;
	Angle = przes;
	//LastTime = Now;
}
